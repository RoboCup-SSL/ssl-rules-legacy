...
Timeouts in extra time follow the same rules as in regular game (stated in \removed{law 7}\added{\autoref{sec:duration-of-the-match}}).
...
This time is suggested to be used by the teams in dialogue with the referee and his assistants to check whether the goalie's position is correct (on the line) and all other rules for penalty can be fullfilled as stated in \removed{law 14}\added{\autoref{sec:penalty-kick}}.
...
For all penalties, the rules of \removed{law 14}\added{\autoref{sec:penalty-kick}} apply.
...
Robots may be exchanged between the kicks following the interchange rules of \removed{law 3}\added{\autoref{sec:number-of-robots}}.
...
\added{If one of the teams abandons the match, before or during its course, the opponent will be awarded winner for all purposes.
...
\added{If the two teams abandon the match, before or during its course, both teams are considered to have lost the match.
Abandoned matches cannot result in ties.}

\added{The competition records will indicate the team(s) that abandoned the match.}

\subsection{Early Termination of Match at Score of 10}

\added{When the score difference reaches 10 goals in a round-robin (not tournament) game, the match is automatically terminated and the team with more goals declared the winner.}
...
\item greatest number of goals scored in all group matches\removed{.}
\end{itemize}
...
\item drawing of lots by the Organising Committee\removed{.}
\end{itemize}
