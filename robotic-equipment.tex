...
The assigned team color is used as the cent\removed{er}\added{re} marker color for all of the team's robots.
The detailed layout of the markers is described in \removed{the following section ``Standard Pattern''}\added{\autoref{subsec:robotic-equipment-standard-pattern}}.
...
The SSL-Vision standard pattern is guaranteed to fit within a circle of radius 85\added{\,}mm that is linearly cut off on the front side of the robot to a distance of 55\added{\,}mm from the cent\removed{er}\added{re}, as shown in \autoref{fig:robothat}.
...
The standard pattern to be used by all teams at RoboCup 2012 is shown in \autoref{fig:stdpattern}.
Note\removed{,} that the organi\removed{z}\added{s}ers reserve the right to change this pattern at any time, if required.
...
\begin{figure}[ht] % ht = here / t = top / b = bottom
	...
	\caption{The Standard Pattern\removed{ for RoboCup 2012}}
...
The cent\removed{er}\added{re} dot color determines the team and is either blue or yellow.
...
Note\removed{,} that the organi\removed{z}\added{s}ers reserve the right to change these color assignments at any time, if required.
...
Teams are encouraged to select color assignments with ID\added{s} 0--7 because they have been experimentally found more stable, as there is no risk that the back two dots ``color-bleed'' into each other.
...
\item
Official colo\removed{u}rs will be provided by the organising committee.
Teams must use the official colo\removed{u}rs unless both teams agree not to.
