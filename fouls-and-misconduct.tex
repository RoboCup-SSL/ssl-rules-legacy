...
A direct free kick is awarded to the opposing team if a robot commits any of the following \removed{four} \added{three} offences:
\begin{itemize}
...
\added{
A penalty kick is also awarded to the opposing team if, while the ball is in play, a defender other than the goalie touches the ball while positioned entirely within the defense area.
}

\removed{
A penalty kick is also awarded to the opposing team if, while the ball is in play, a defender other than the goalie enters its own defense area and, while infringing on that area, the ball either touches the infringing robot or is located within the defense area.
}
...
\item touche\removed{d}\added{s} the ball such that the top of the ball travels more than 150\,mm from the ground, and the ball subsequently enters their opponent's goal, without having either touched a teammate (while below 150\,mm) or remained in contact with the ground (stopped bouncing).
...
\item \added{touches the ball such that the top of the ball travels more than 150\,mm from the ground, and the ball subsequently crosses the midline, without having either touched another robot or the ground}
...
A team is cautioned and shown the yellow card if a robot on that team commits any of the following \removed{six} offences:
\begin{enumerate}
...
\item \added{is a robot other than the goalie, and touches the ball while in play, while being located partially but not entirely within the defense area}
\item \added{travels faster than 1\,m/s while the ball is out of play, no restart of play has been called, and the game is in neither a time-out, half-time, nor similar break in gameplay such as the interval before extra time or a penalty shootout (for games on the double-size field, the speed limit is increased to 1.5\,m/s)}
\end{enumerate}
...
\added{It is recommended that the assistant referee be responsible for observing the robots and notifying the main referee when a yellow card should be issued for violent contact.
...
\item
\added{The robot speed limit described in \autoref{subsec:fouls-and-misconduct-disciplinary-sanctions} applies only to cases where the Referee Box is reporting the STOP command during normal play or a penalty shootout.
