...
\item kicks the ball such that it exceeds \removed{8} \added{6.5}\,m/s in \added{absolute} speed \added{(in 3D-space) as measured by the autonomous referees}
...
\item persistently \added{(three times in a row)} infringes the Laws of the Game
...
\item \removed{repeatedly} travels faster than 1.5\,m/s \added{three times in a row} while the ball is
...
extra time or a penalty shoot\added{-}out
\end{enumerate}
...
After two minutes of play (as measured by the \removed{assistant referee}\added{Referee Box} using the official game time), the yellow card expires and the number of robots allowed increases by one.
...
\added{For every third yellow card for one team, a penalty kick is awarded to the opponent team.}
...
It is recommended that the assistant referee \added{and Referee Box operator} be responsible for observing the robots and notifying the main referee when a yellow card should be issued for violent contact.
...
\begin{figure}[ht] % ht = here / t = top / b = bottom
	...
	\caption{The \removed{80/}20\added{\%} ball covering/holding rule}
	\label{fig:20-rule}
\end{figure}
...
\item
\added{
The intention of the robot speed limit described in \autoref{subsec:fouls-and-misconduct-disciplinary-sanctions} is to avoid collisions caused by large numbers of robots moving long distances and to avoid robots accidentally interfering with the referee controlling the ball.
It also reduces situations where robots accidentally interfere with the automatic ball placement.
}
\removed{
