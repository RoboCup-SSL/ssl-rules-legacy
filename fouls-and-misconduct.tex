...
\begin{enumerate}
\item
Substantial contact is contact sufficient to dislodge the robot from its current orientation, position, or motion in the case where it is moving. \added{To be more exact: At the moment of collision, the difference of the speed vectors of both robots is taken and projected onto the line that is defined by the position of both robots. If the length of this projection is greater than 1.5m/s, the faster robot committed a foul. If both robots have the same speed, both conduct a foul and the game will be restarted with a forced start.}
