...
\ifdiff
\item \removed{is the second defending robot to simultaneously occupy the team's defence area in such a way to substantially affect game play}
...
A penalty kick is awarded if any of the \removed{above four} offences \added{listed in \autoref{subsec:fouls-and-misconduct-direct-free-kick}} is committed by a robot inside his own defence area, irrespective of the position of the ball, provided it is in play.

\added{A penalty kick is also awarded to the opposing team if, while the ball is in play, a defender other than the goalie enters its own defense area and, while infringing on that area, the ball either touches the infringing robot or is located within the defense area.}
...
\ifdiff
\item \removed{touched the ball such that the top of the ball travels more than 150\,mm from the ground, and the ball subsequently enters their opponent's goal, without having either touched a teammate (while below 150\,mm) or remained in contact with the ground (stopped bouncing).}
\fi
\item \added{is the last robot to touch the ball before it touches the opposing goalkeeper, then the ball touches the opposing goalkeeper while more than 150\,mm from the ground, then the ball enters the opposing goal without subsequently touching any other robot except the opposing goalkeeper}
...
\ifdiff
\item
\removed{The current rule about scoring after chip kicks is defined in this section (subsection Indirect Free Kicks) only.
...
\begin{itemize}
\item \removed{If a robot chips the ball (no matter at which height it travels) at a team mate and the ball subsequently enters the own goal, the opponent team scores.}
\item \removed{If a robot chips the ball at an opponent and the ball subsequently enters the own goal while staying below 150mm all the time after touching the opponent robot, the opponent team also scores.}
\item \removed{If a robot chips the ball at an opponent and the ball subsequently enters the own goal after having been above 150mm for some time (and not being in constant touch with the ground afterwards) after touching the opponent robot, the opponent team does not score.}
\end{itemize}
\fi
