...
\begin{enumerate}
\item A goal is only scored if the ball has not exceeded a robot height (150mm) between the last kick of an attacker and the time the ball crossed the goal line. The intention of this rule is to reduce `blind` chip kicks on the goal.
\end{enumerate}
}
