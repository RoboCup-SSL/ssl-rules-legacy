...
follows\added{.}\removed{for the two field sizes:}
\removed{
\begin{itemize}
...
length 500\,mm parallel to the goal line. Figure \ref{fig:double_size_field}
shows this configuration.}
The area bounded by \removed{this arc}\added{these arcs} and the goal line is
the defence area.
...
A \removed{mounting bar}\added{pair of mounting bars} will be provided 4\,m
above the field. \removed{The bar will run above the midline of the field from
goal to goal.}\added{The bars will run parallel to the goal lines above the
center of each field half.} The \removed{bar}\added{bars} should be mounted
securely so that \removed{it does}\added{they do} not swing or sway under a
small external force, and \removed{it}\added{they} should not bend or twist
