...
\added{of nominal size 9000\,mm$\times$6000\,mm.
The exact field dimensions and the field markings at the venue may vary by up
to $\pm10\%$ in each linear dimension.}
\removed{
Teams have the option of choosing to play on one of two sizes of fields:
\begin{enumerate}
\item \textbf{Single-size field}, of size 6050\,mm$\times$4050\,mm
\item \textbf{Double-size field}, of size 8090\,mm$\times$6050\,mm
\end{enumerate}
}
The dimensions include boundary lines.
Dimensions of the field, goals, and special field areas are in millimetres and
are shown in
\removed{\autoref{fig:single_size_field} (single-size field) and}
\autoref{fig:double_size_field}
\removed{(double-size field)}.
...
\begin{figure}[ht] % ht = here / t = top / b = bottom
  ...
  \caption{The field dimensions \removed{of the double-size field}}
  \label{fig:double_size_field}
\end{figure}

\removed{
The rules in this rule book apply identically to both sizes of field, unless
otherwise explicitly mentioned. Specifically, the following rules have
differences in their application to the two field sizes:
\begin{enumerate}
  \item The dimensions of the field and the field markings in
  \autoref{sec:field-of-play}
  \item The number and length of timeouts in
    \autoref{subsec:duration-of-the-match-timeouts}
  \item The robot speed limit during stoppage of play in
    \autoref{subsec:fouls-and-misconduct-disciplinary-sanctions}
\end{enumerate}
}
...
The field surface will continue for \removed{675\,mm}\added{700\,mm} beyond
the boundary lines on all sides.
The outer \removed{425\,mm}\added{450\,mm} of this runoff area is used as a
...
The field of play is divided into two halves by a halfway line \added{that runs
along the width of the field and through the center of the field}.

\added{A mid-line runs along the length of the field, passing through the center
...
\removed{for the two field sizes}:
\removed{
\begin{itemize}
...
\added{Two quarter circles of radius of 1000\,mm
are drawn on the field of play. These quarter circles are connected by a line of
length 500\,mm parallel to the goal line. Figure \ref{fig:double_size_field}
shows this configuration.}
...
The distance between the side walls is\removed{700\,mm for the single-size field
and} 1000\,mm\removed{for the double-size field}, and the goal is 180\,mm deep.
...
\added{
A pair of mounting bars will be provided 4\,m above the field. The bars will run
parallel to the goal lines above the center of each field half. The bar should
be mounted securely so that it does not swing or sway under a small external
force, and it should not bend or twist significantly when the weight of typical
video equipment is added.
}
\removed{
A mounting bar will be provided 4\,m above the field. The bar will run above the
midline of the field from goal to goal. The bar should be mounted securely so
that it does not swing or sway under a small external force, and it should not
bend or twist significantly when the weight of typical video equipment is added.
}
...
\added{The vision patterns on the top of the robots must adhere to the
